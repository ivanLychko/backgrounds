class CrackInjectionOrangeBackground {
  ...
  animate() {
    ...
    this.injections.forEach((injection, injIndex) => {
      ...
      if (injection.life <= 0 || injection.radius > injection.maxRadius) {
        ...
        // Оранжевый/красный градиент инжекции
        const gradient = this.ctx.createRadialGradient(
          injection.x, injection.y, 0,
          injection.x, injection.y, injection.radius
        );
        gradient.addColorStop(0, `rgba(255, 150, 100, ${injection.life * 0.8})`);
        gradient.addColorStop(0.6, `rgba(240, 130, 80, ${injection.life * 0.6})`);
        gradient.addColorStop(1, `rgba(220, 110, 60, 0)`);
        ...
        injection.particles.forEach(particle => {
          if (particle.distance < particle.maxDistance) {
            ...
            this.ctx.fillStyle = `rgba(255, 150, 100, ${injection.life * 0.7})`;
            this.ctx.beginPath();
    ...
    this.cracks.forEach(crack => {
      ...
      // Оранжевые/красные трещины
      this.ctx.strokeStyle = `rgba(200, 100, 80, ${0.6 * appearProgress})`;
      ...
      // Заполненные части - яркий оранжевый
      this.ctx.strokeStyle = 'rgba(255, 180, 120, 0.5)';
...
export default CrackInjectionOrangeBackground;
