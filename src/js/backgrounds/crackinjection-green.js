class CrackInjectionGreenBackground {
  ...
  animate() {
    ...
    this.injections.forEach((injection, injIndex) => {
      ...
      if (injection.life <= 0 || injection.radius > injection.maxRadius) {
        ...
        // Зеленый градиент инжекции
        const gradient = this.ctx.createRadialGradient(
          injection.x, injection.y, 0,
          injection.x, injection.y, injection.radius
        );
        gradient.addColorStop(0, `rgba(100, 220, 150, ${injection.life * 0.8})`);
        gradient.addColorStop(0.6, `rgba(80, 200, 130, ${injection.life * 0.6})`);
        gradient.addColorStop(1, `rgba(60, 180, 110, 0)`);
        ...
        injection.particles.forEach(particle => {
          if (particle.distance < particle.maxDistance) {
            ...
            this.ctx.fillStyle = `rgba(100, 220, 150, ${injection.life * 0.7})`;
            this.ctx.beginPath();
    ...
    this.cracks.forEach(crack => {
      ...
      // Зеленые трещины
      this.ctx.strokeStyle = `rgba(80, 150, 100, ${0.6 * appearProgress})`;
      ...
      // Заполненные части - яркий зеленый
      this.ctx.strokeStyle = 'rgba(120, 240, 160, 0.5)';
