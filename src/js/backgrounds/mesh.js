    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        this.points.push({
          ...
          originalX: col * spacingX + spacingX / 2,
          originalY: row * spacingY + spacingY / 2,
        });
      }
    }
  }
  ...
  animate() {
    ...
    this.points.forEach(point => {
      ...
      if (distance < maxDist) {
        ...
        point.x = point.originalX + (dx / distance) * force * 50;
        point.y = point.originalY + (dy / distance) * force * 50;
      } else {
    ...
    this.points.forEach((point, i) => {
      this.points.slice(i + 1).forEach(other => {
