class EpoxyInjectionBackground {
    ...
    this.streams.forEach((stream, streamIndex) => {
      ...
      stream.particles.forEach((particle, index) => {
        ...
        if (particle.life <= 0 || particle.y > this.canvas.height) {
          ...
          // Draw epoxy particle (darker, more viscous)
          this.ctx.fillStyle = `rgba(100, 100, 120, ${particle.life * 0.8})`;
          this.ctx.beginPath();
      ...
      if (stream.particles.length === 0 && stream.life <= 0) {
...
export default EpoxyInjectionBackground;
