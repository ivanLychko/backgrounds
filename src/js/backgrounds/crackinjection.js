class CrackInjectionBackground {
  ...
  animate() {
    ...
    this.injections.forEach((injection, injIndex) => {
      ...
      if (injection.life <= 0 || injection.radius > injection.maxRadius) {
        ...
        gradient.addColorStop(0.6, `rgba(180, 160, 130, ${injection.life * 0.6})`);
        ...
        injection.particles.forEach(particle => {
          if (particle.distance < particle.maxDistance) {
            ...
            this.ctx.fillStyle = `rgba(200, 180, 150, ${injection.life * 0.7})`;
            this.ctx.beginPath();
    ...
    this.cracks.forEach(crack => {
      ...
      this.ctx.strokeStyle = `rgba(100, 100, 100, ${0.6 * appearProgress})`;
      ...
      // Draw filled parts in different color
      this.ctx.strokeStyle = 'rgba(200, 180, 150, 0.4)';
