class CrackInjectionPurpleBackground {
  ...
  animate() {
    ...
    this.injections.forEach((injection, injIndex) => {
      ...
      if (injection.life <= 0 || injection.radius > injection.maxRadius) {
        ...
        // Фиолетовый/розовый градиент инжекции
        const gradient = this.ctx.createRadialGradient(
          injection.x, injection.y, 0,
          injection.x, injection.y, injection.radius
        );
        gradient.addColorStop(0, `rgba(200, 120, 255, ${injection.life * 0.8})`);
        gradient.addColorStop(0.6, `rgba(180, 100, 240, ${injection.life * 0.6})`);
        ...
        injection.particles.forEach(particle => {
          if (particle.distance < particle.maxDistance) {
            ...
            this.ctx.fillStyle = `rgba(200, 120, 255, ${injection.life * 0.7})`;
            this.ctx.beginPath();
    ...
    this.cracks.forEach(crack => {
      ...
      // Фиолетовые трещины
      this.ctx.strokeStyle = `rgba(150, 100, 200, ${0.6 * appearProgress})`;
      ...
      // Заполненные части - яркий розовый/фиолетовый
...
export default CrackInjectionPurpleBackground;
