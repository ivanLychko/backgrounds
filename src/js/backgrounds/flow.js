class FlowBackground {
  ...
  getFlowField(x, y) {
    const angle = Math.sin(x * 0.01 + this.time) * Math.cos(y * 0.01 + this.time) * Math.PI * 2;
    const mouseX = this.mouse.x * this.canvas.width;
    const mouseY = this.mouse.y * this.canvas.height;
    const dx = mouseX - x;
    const dy = mouseY - y;
    const dist = Math.sqrt(dx * dx + dy * dy);
    const mouseAngle = dist < 200 ? Math.atan2(dy, dx) : 0;
    
    return angle + mouseAngle * 0.5;
  }
  ...
  animate() {
    ...
    this.particles.forEach(particle => {
      const angle = this.getFlowField(particle.x, particle.y);
